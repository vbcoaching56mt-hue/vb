...
import { supabase } from './supabaseClientConfig'; // archive - supabaseAdmin supprimé
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/70 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 w-full max-w-lg border border-gray-100">
        <h3 className="text-xl font-extrabold text-gray-900 mb-2">├ëmargement ├ëlectronique</h3>
        <p className="text-sm text-gray-500 mb-6">Veuillez signer lisiblement dans le cadre ci-dessous pour valider votre pr├®sence ou votre accord.</p>
    ...
    <div className="fixed inset-0 bg-gray-900/80 z-[100] flex items-center justify-center p-2 md:p-8 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col relative overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-white z-10 shrink-0">
          ...
          <button onClick={onClose} className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 transition-colors">Ô£ò</button>
        </div>
        <div className="flex-1 bg-gray-100 p-2 md:p-6 overflow-hidden">
          {document.url ? (
            ...
            <div className="flex items-center justify-center h-full text-gray-500 font-medium">Aucun fichier joint ├á ce document.</div>
          )}
        </div>
        {document.signe_par_client && (
          <div className="bg-green-50 border-t border-green-200 p-4 shrink-0 flex items-center">
            <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center text-green-600 mr-4 font-bold text-xl">Ô£ô</div>
            <div className="flex-1">
              <p className="font-bold text-green-800 text-lg">Document valid├® num├®riquement</p>
              <p className="text-sm text-green-700 mt-0.5">Certifi├® sign├® le {document.date_signature_client ? new Date(document.date_signature_client).toLocaleString('fr-FR') : 'Date inconnue'} {document.signature_client_url ? '(Signature attach├®e)' : ''}</p>
              {document.signature_client_url && <a href={document.signature_client_url} target="_blank" rel="noreferrer" className="text-xs text-green-600 underline font-medium mt-1 inline-block">Voir le trac├® de la signature</a>}
            </div>
          </div>
        )}
      </div>
    ...
    <div className="fixed inset-0 bg-gray-900/70 z-[150] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 w-full max-w-md border border-gray-100 animate-slide-up">
        ...
        <p className="text-gray-500 mb-8">
          Confirmez-vous la suppression d├®finitive de <span className="font-bold text-gray-900">{itemName}</span> ? 
          Toutes les donn├®es associ├®es seront supprim├®es et cette action est irr├®versible.
        </p>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in">
      <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl border border-gray-100 overflow-hidden animate-slide-up">
        <div className="bg-indigo-600 p-8 text-white relative">
          <h2 className="text-2xl font-black">Ajouter un ├®l├®ment</h2>
          <p className="text-indigo-100 text-sm mt-1 opacity-80">Configurez les d├®tails de l'activit├®.</p>
          <button onClick={onClose} className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-all">Ô£ò</button>
        </div>
        ...
        <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            {['signature', 'document', 'exercice'].map(t => (
              <button
                ...
                onClick={() => {
                  ...
                  if (t === 'signature') {
                    setTitle('├ëmargement de pr├®sence');
              ...
              >
                <span className="text-2xl">{t === 'signature' ? 'Ô£ì´©Å' : t === 'document' ? '­ƒôä' : 'ÔÜÖ´©Å'}</span>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Libell├® de l'activit├®</label>
            <input
              type="text"
              placeholder={type === 'signature' ? '├ëmargement de pr├®sence' : type === 'exercice' ? 'Nom de l\'exercice' : 'Titre du document'}
              value={title}
            ...
            <div className="space-y-4 animate-slide-up">
              <div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <option value="">S├®lectionner dans la mod├®loth├¿que...</option>
                  ...
                  <label className={`w-full flex items-center justify-center gap-2 p-4 rounded-2xl border-2 border-dashed border-gray-200 text-gray-500 font-bold text-xs cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-all ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    {isUploading ? 'Importation en cours...' : '­ƒôü Importer un fichier (PDF, Word, Excel)'}
                    <input
                      type="file"
                      className="hidden"
                  ...
                  {selectedResourceId && !isUploading && (
                    <p className="text-[10px] text-green-600 font-bold flex items-center gap-1">
                      Ô£ô Fichier li├® : {selectedResourceId}
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="bg-indigo-50/50 p-4 rounded-2xl space-y-3">
                  ...
                  <div className="flex gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-sm font-bold text-gray-700">├Ç signer</span>
                    </label>
                  </div>
          ...
          <div className="pt-4 pb-2">
            ...
            >
              {isUploading ? 'Veuillez patienter...' : 'Enregistrer l\'activit├®'}
            </button>
          </div>
        </div>
      </div>
...
const LoginView = ({ handleLogin, supabase, successMessage }) => {
  const [email, setEmail] = useState(() => {
    // Tenter de pr├®-remplir l'email depuis l'URL (#email=... ou ?email=...)
  ...
  const handleSubmit = async (e) => {
    ...
    if (authError) {
      ...
      if (authError.message.includes('Invalid login credentials')) {
        ...
        setErrorMsg('Votre email n\'a pas encore ├®t├® confirm├®.');
      } else {
    ...
    // 2. Chercher le r├┤le dans la base de donn├®es
    ...
    if (userEmail) {
      ...
      // Si pas trouv├®, chercher dans la table 'clients' (insensible ├á la casse)
      ...
      if (clientData) {
        ...
        console.error('Utilisateur non trouv├® dans les DBs:', dbError || clientError);
        setErrorMsg('Votre compte existe mais n\'est pas encore configur├® dans les tables de donn├®es. Contactez l\'administrateur.');
      }
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
        <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-md text-center border border-gray-100 animate-fade-in">
          ...
          <h1 className="text-2xl font-extrabold text-gray-900 mb-2">Mot de passe oubli├®</h1>
          <p className="text-gray-500 mb-8">Saisissez votre email pour r├®initialiser l'acc├¿s.</p>

          {resetSent ? (
            <div className="bg-green-50 text-green-700 p-4 rounded-xl text-sm font-medium mb-6">
              Un email contenant le lien de r├®initialisation vous a ├®t├® envoy├®.
            </div>
          ) : (
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-md text-center border border-gray-100 animate-fade-in">
        <div className="w-20 h-20 bg-rose-500 rounded-2xl flex items-center justify-center text-white text-3xl font-black mx-auto mb-6 shadow-lg shadow-rose-500/30">VB</div>
        <h1 className="text-2xl font-extrabold text-gray-900 mb-2">Connexion ├á VB ERP</h1>
...
const ClientDetailView = ({
  ...
  const handleSaveClientInfo = async () => {
    ...
    const { error } = await supabase.from('clients').upsert({
      ...
      module_id: client.module_id, // Pr├®server le module
      formateur_id: client.formateur_id // H├®rit├® de l'assignation globale
    }, { onConflict: 'id' });
    ...
    if (error) {
      ...
      // D├®clenchement automatique des s├®ances si un module est pr├®sent
      if (client.module_id) {
        console.log("[handleSaveClientInfo] Module d├®tect├®, lancement de generateSessions pour client:", client.id);
        await generateSessions(client);
      }
      alert("Informations personnelles sauvegard├®es avec succ├¿s !");
    }
  ...
  const handleAddCustomSession = async () => {
    ...
    const { error } = await supabase.from('sessions').insert([{
      ...
      titre: `S├®ance personnalis├®e ${nextNum}`,
      statut: '├Ç venir'
    }]);
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      <button onClick={onBack} className="text-gray-500 hover:text-gray-900 font-bold flex items-center mb-4 transition-colors">
        <ChevronLeft className="w-5 h-5 mr-1" /> Retour ├á la supervision
      </button>
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between md:items-center">
        <div>
          ...
          <p className="text-gray-500">{client.email || "Aucun email"} - N┬░ Dossier: {client.numero_dossier || "Non d├®fini"}</p>
        </div>
      ...
      <div className="flex gap-4 border-b border-gray-200">
        <button onClick={() => setActiveTab('infos')} className={`px-4 py-3 font-bold text-sm ${activeTab === 'infos' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-800'}`}>Infos & Modalit├®s</button>
        <button onClick={() => setActiveTab('seances')} className={`px-4 py-3 font-bold text-sm ${activeTab === 'seances' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-800'}`}>Supervision S├®ances</button>
        <button onClick={() => setActiveTab('docs')} className={`px-4 py-3 font-bold text-sm ${activeTab === 'docs' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-800'}`}>Documents li├®s</button>
      </div>
      ...
      {activeTab === 'infos' && (
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1">Module Assign├®</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1">Formateur Assign├®</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input className="w-full p-3 text-sm border bg-gray-50 border-gray-200 focus:border-indigo-500 rounded-xl outline-none transition-colors" value={clientInfo.nomcomplet_client} onChange={e => setClientInfo({ ...clientInfo, nomcomplet_client: e.target.value })} placeholder="Nom Complet (G├®n├®ration Docs)" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">T├®l├®phone</label>
              <input className="w-full p-3 text-sm border bg-gray-50 border-gray-200 focus:border-indigo-500 rounded-xl outline-none transition-colors" value={clientInfo.client_phone} onChange={e => setClientInfo({ ...clientInfo, client_phone: e.target.value })} placeholder="T├®l├®phone" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">Adresse Postale</label>
              <input className="w-full p-3 text-sm border bg-gray-50 border-gray-200 focus:border-indigo-500 rounded-xl outline-none transition-colors" value={clientInfo.adresse_client} onChange={e => setClientInfo({ ...clientInfo, adresse_client: e.target.value })} placeholder="Adresse compl├¿te" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">Montant de la Prestation (Ôé¼)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">Modalit├®s de la formation</label>
          ...
          <h3 className="text-lg font-bold text-gray-800 mt-2">G├®n├®rateurs Automatiques</h3>
          <div className="flex gap-2 flex-wrap pt-2">
            {Object.keys(documentTemplates || {}).map(key => (
              <button key={key} onClick={() => handleGenerateDocx(client, key)} className="bg-indigo-50 flex items-center text-indigo-700 px-4 py-2 rounded-xl text-sm font-bold border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all">
                <FileText className="w-4 h-4 mr-2" /> G├®n├®rer {key}
              </button>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'seances' && (
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-gray-800">Calendrier des S├®ances</h3>
            <button onClick={handleAddCustomSession} className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-2 shadow-sm transition-all">
              <Plus size={14} /> Ajouter une ├®tape personnalis├®e
            </button>
          </div>
          <div className="space-y-4">
              ...
              <div key={session.id} className="p-5 border border-gray-100 rounded-2xl bg-gray-50 flex flex-col gap-4 relative group">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Titre de la s├®ance / ├®tape</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Type d'Activit├®</label>
                    <select
                    ...
                    >
                      <option value="Signature">Signature Pr├®sence</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2 border-t border-gray-200/50">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Ressource P├®dagogique (Mod├®loth├¿que)</label>
                    <select
                    ...
                    >
                      <option value="">Aucune ressource li├®e</option>
            ...
            )) : <p className="text-gray-500 italic text-sm text-center py-8">Aucune s├®ance n'est encore programm├®e pour ce client.</p>}
          </div>
        </div>
      )}
      ...
      {activeTab === 'docs' && (
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
          <h3 className="text-lg font-bold text-gray-800 mb-6">Documents Upload├®s</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            )) : <p className="text-gray-500 italic text-sm">Aucun document rattach├®.</p>}
          </div>
        </div>
      )}
    </div>
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 mb-8">
        ...
        <div className="space-y-8">
          {Object.keys(clientsGroupedByFormateur).map(fId => {
            ...
            const formateurName = formateur ? `${formateur.nom || ''} ${formateur.prenom || ''}`.trim() : 'NON ASSIGN├ë';
            return (
              <div key={fId} className="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex items-center">
                  ...
                  <span className="ml-auto text-xs font-bold text-gray-500">{clientsGroupedByFormateur[fId].length} b├®n├®ficiaire(s)</span>
                </div>
                    ...
                    <li key={client.id} className="p-4 hover:bg-gray-50/50 transition-colors flex items-center justify-between cursor-pointer" onClick={() => setExpandedClientId(client.id)}>
                      <div className="flex items-center">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-500">{client.email || 'Email non renseign├®'}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
...
const AdminFormateursView = ({ clients, formateurs, documents, expandedClientId, setExpandedClientId, supabase, fetchUtilisateurs, fetchDocuments, activeTab, setActiveTab, modules, sessions, handleDownloadResource, handleDeleteFormateur }) => {
  ...
  // --- Modal de R├®sum├® de Planning pour l'Admin ---
          ...
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50 flex-shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mt-1.5">Planning & ├ëmargements Qualiopi</p>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/30">
            {clientSessions.length > 0 ? (
              <table className="w-full text-left border-collapse bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 text-sm">
                <thead>
                  <tr className="bg-gray-100/50 text-[10px] font-black uppercase text-gray-400 tracking-widest">
                    <th className="p-4">S├®ance</th>
                    <th className="p-4">Date & Heures</th>
                    <th className="p-4">Activit├®</th>
                    <th className="p-4 text-center">├ëmargement Client</th>
                    <th className="p-4 text-center">├ëmargement Coach</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {clientSessions.map(s => (
                    <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="font-bold text-gray-700">{s.date ? new Date(s.date).toLocaleDateString() : 'Non planifi├®'}</div>
                      ...
                      <td className="p-4 text-center">
                        <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase ${s.statut_client === 'Sign├®' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                          {s.statut_client === 'Sign├®' ? 'OK Ô£ô' : 'Attente'}
                        </span>
                      </td>
                      <td className="p-4 text-center">
                        <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase ${s.statut_formateur === 'Sign├®' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                          {s.statut_formateur === 'Sign├®' ? 'OK Ô£ô' : 'Attente'}
                        </span>
                      </td>
                    </tr>
                  ))}
              ...
              <div className="py-20 text-center bg-white rounded-3xl border border-dashed border-gray-200">
                <p className="text-gray-400 italic">Aucune s├®ance n'a ├®t├® g├®n├®r├®e pour ce client.</p>
              </div>
            )}
          </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200 animate-slide-up space-y-6">
                    <div>
                      <h4 className="text-xs font-bold text-gray-500 mb-4 uppercase tracking-wider flex items-center">
                        <Users className="w-3.5 h-3.5 mr-2" /> Clients Assign├®s
                      </h4>
                      {sesClients.length > 0 ? (
                              ...
                              {sesClients.map(client => {
                                ...
                                return (
                                  <tr key={client.id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="p-3">
                                      <span className="text-xs font-medium text-gray-600">{clientModule?.nom || 'Non assign├®'}</span>
                                    </td>
                        ...
                        <div className="py-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
                          <p className="text-sm text-gray-400 italic">Aucun client assign├® ├á ce formateur.</p>
                        </div>
                      )}
                    </div>
                  </div>
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-5xl mx-auto">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Ing├®nierie Formation</h1>
          <p className="text-gray-500 text-lg mt-1">G├®rez la structure de vos modules et s├®ances.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100">
        ...
        <form onSubmit={handleAddModule} className="flex flex-col md:flex-row gap-4 items-end mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200">
          ...
          <div className="w-32">
            <label className="block text-sm font-medium text-gray-700 mb-1">S├®ances pr├®vues</label>
          ...
          <button type="submit" className="bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg px-6 py-2.5 shadow-sm transition-colors">Cr├®er Module</button>
        </form>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {modules.map(mod => {
            ...
            return (
              <div key={mod.id} className="border border-purple-100 bg-purple-50/20 p-5 rounded-2xl relative shadow-sm h-fit">
                ...
                <span className="text-xs font-bold text-purple-700 bg-purple-100 px-2 py-1.5 rounded-xl absolute top-5 right-5">{mod.seances_prevues} S├®ance(s)</span>
                ...
                <ul className="space-y-2 mb-4">
                  ...
                  {docs.length === 0 && <li className="text-xs text-gray-400 italic">Aucun document type li├®.</li>}
                </ul>
                  ...
                  <form onSubmit={(e) => handleLinkDocument(e, mod)} className="bg-white p-4 rounded-xl shadow-sm border border-purple-200 flex flex-col gap-3 animate-fade-in">
                    ...
                    <div className="flex gap-2">
                      <select value={newModDocType} onChange={e => setNewModDocType(e.target.value)} className="flex-1 text-sm p-2 border border-gray-200 rounded-lg outline-none focus:border-purple-500">
                        <option value="Autre">Autre</option><option value="Contrat">Contrat</option><option value="├ëvaluation">├ëvaluation</option>
                      </select>
                      ...
                      <button type="button" onClick={() => setAddingToModuleId(null)} className="text-gray-400 hover:text-gray-600 px-2 shrink-0">Ô£ò</button>
                    </div>
                  </form>
                ) : (
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setModelingModuleId(modelingModuleId === mod.id ? null : mod.id)} className="text-xs font-bold text-indigo-600 hover:text-white hover:bg-indigo-600 flex items-center bg-white border border-indigo-200 px-4 py-2 rounded-lg transition-all">ÔÜÖ´©Å Mod├®liser Parcours</button>
                  </div>
                )}

                {/* Interface de Mod├®lisation du Parcours */}
                {modelingModuleId === mod.id && (
                  <div className="mt-6 pt-6 border-t border-purple-100 animate-fade-in space-y-6">
                    <h4 className="text-sm font-bold text-indigo-700 flex items-center gap-2">
                      <Layout size={16} /> Mod├®lisation du Parcours
                    </h4>
                    ...
                    <div className="space-y-4">
                            ...
                            <div className="bg-indigo-50/50 p-4 flex items-center justify-between border-b border-indigo-50">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] text-gray-400 uppercase font-black">Dossier S├®ance</p>
                                </div>
                              </div>
                              <button
                                ...
                                <div key={res.id} className="flex items-center justify-between bg-gray-50/50 p-3 rounded-xl border border-gray-100 text-[11px]">
                                  <div className="flex items-center gap-3">
                                    <span className="text-gray-400">
                                      {res.type === 'signature' ? 'Ô£ì´©Å' : res.type === 'document' ? '­ƒôä' : 'ÔÜÖ´©Å'}
                                    </span>
                                    <div className="flex flex-col">
                      ...
                      {templates.length === 0 && (
                        <div className="bg-gray-50 border border-dashed border-gray-200 p-8 rounded-[32px] text-center">
                          <p className="text-gray-400 text-sm italic">Aucun dossier cr├®├®.</p>
                        </div>
                      )}
                    </div>
          ...
          {modules.length === 0 && <div className="text-gray-500 italic col-span-2">Cr├®ez votre premier module.</div>}
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center">
          <span className="w-2 h-6 bg-emerald-500 rounded-full mr-3"></span> Ressources P├®dagogiques
        </h2>
      ...
      {/* Gestion des Mod├¿les DOCX */}
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center">
          <span className="w-2 h-6 bg-blue-500 rounded-full mr-3"></span> Biblioth├¿que de Mod├¿les Word
        </h2>
        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
          <h3 className="font-bold text-gray-900 mb-4 flex items-center"><FileText className="mr-2" size={18} /> Nouveau Mod├¿le</h3>
          <div className="space-y-4">
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-5xl mx-auto">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Mes Clients Assign├®s</h1>
          <p className="text-gray-500 text-lg mt-1">Suivez l'avancement et g├®rez les ├®margements des sessions.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {assignedClients.length > 0 ? assignedClients.map(client => {
            ...
            <div key={client.id} className={`bg-white rounded-3xl p-6 shadow-sm border ${isExpanded ? 'border-indigo-200' : 'border-gray-100'} transition-all`}>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {isExpanded ? "R├®duire" : "G├®rer les s├®ances"}
                  </button>
                </div>
              </div>
                ...
                <div className="mt-8 pt-8 border-t border-gray-100 animate-slide-up">
                  <div className="flex items-center gap-3">
                    <h4 className="font-bold text-gray-800 flex items-center">
                      ...
                      Planning des S├®ances - {assignedModule?.nom || 'Sans module'}
                    </h4>
                    ...
                    {(userRole === 'admin' || userRole === 'formateur') && (
                      ...
                      >
                        <span className="mr-1.5">Ô×ò</span> Ajouter une s├®ance
                      </button>
                    )}
                  </div>

                  {/* Espace de g├®n├®ration des documents administratifs (Optionnel) */}
                  <div className="mb-4 flex flex-wrap gap-2">
                    {/* On pourra ajouter ici des boutons d'actions globales si n├®cessaire */}
                  </div>
                  ...
                  {clientSessions.length > 0 ? (
                    <div className="overflow-hidden rounded-2xl border border-gray-100">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 text-gray-400 font-bold uppercase text-[10px] tracking-widest">
                          <tr>
                            <th className="px-4 py-3 text-left">N┬░ & S├®ance</th>
                            <th className="px-4 py-3 text-left">Date</th>
                            <th className="px-4 py-3 text-left">Horaires (D├®but/Fin)</th>
                                ...
                                <tr className="bg-indigo-50/30">
                                  <td className="px-4 py-3 font-black text-indigo-900 border-l-4 border-indigo-500">
                                    <div className="flex items-center gap-2 text-xs">
                                      ...
                                      <span>S├ëANCE {group.numero} : {group.nom}</span>
                                    </div>
                                  </td>
                                  ...
                                  <td colSpan="2" className="px-4 py-3 text-right">
                                    <span className="text-[9px] font-black text-indigo-400 uppercase tracking-tighter">Container S├®ance</span>
                                  </td>
                                </tr>
                                  ...
                                  <tr key={session.id} className="hover:bg-gray-50/30 transition-colors border-l border-gray-100">
                                    <td className="px-8 py-3 italic text-gray-600 text-[11px] flex items-center gap-2">
                                      <span className="text-[14px]">
                                        {session.type_activite === 'signature' ? 'Ô£ì´©Å' : session.type_activite === 'document' ? '­ƒôä' : 'ÔÜÖ´©Å'}
                                      </span>
                                      <span className="truncate">{session.ressource_titre || session.nom}</span>
                                    </td>
                                    <td colSpan="2" className="px-4 py-3 text-[10px] text-gray-400 italic">H├®rit├® du dossier</td>
                                    <td className="px-4 py-3">
                                      <div className="flex flex-col gap-1">
                                        <div className="flex items-center gap-1.5">
                                          <span className={`w-1.5 h-1.5 rounded-full ${session.statut_client === 'Sign├®' ? 'bg-green-500' : 'bg-orange-400'}`}></span>
                                          <span className="text-[8px] font-black uppercase text-gray-500">Client: {session.statut_client || (session.statut === 'Sign├®' ? 'Sign├®' : '├Ç venir')}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                          <span className={`w-1.5 h-1.5 rounded-full ${session.statut_formateur === 'Sign├®' ? 'bg-green-500' : 'bg-orange-400'}`}></span>
                                          <span className="text-[8px] font-black uppercase text-gray-500">Coach: {session.statut_formateur || (session.type_activite === 'signature' && session.statut === 'Sign├®' ? 'Sign├®' : '├Ç venir')}</span>
                                        </div>
                                      </div>
                                    </td>
                                      ...
                                      <div className="flex justify-end items-center gap-2">
                                        {(() => {
                                          ...
                                          if (session.type_activite === 'signature') {
                                            const isSigned = session.statut_formateur === 'Sign├®';
                                            return (
                                              <button
                                              ...
                                              >
                                                {isSigned ? '├ëmarg├® Ô£ô' : isDateLocked ? `Indisponible` : '├ëmarger'}
                                              </button>
                                            );
                                          }
                                          ...
                                          if (session.type_activite === 'document') {
                                            ...
                                            return (
                                              <div className="flex gap-2">
                                                ...
                                                {isToSign && (
                                                  <button
                                                    disabled={session.statut === 'Sign├®' || isDateLocked}
                                                    onClick={() => signSession(session)}
                                                    className={`text-[10px] font-bold px-3 py-1.5 rounded-lg border transition-all ${(session.statut === 'Sign├®') ? 'bg-green-50 text-green-600 border-green-200' : isDateLocked ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-indigo-600 text-white border-indigo-700 hover:bg-indigo-800'}`}
                                                  >
                                                    {session.statut_formateur === 'Sign├®' ? 'Sign├® Ô£ô' : 'Signer Document'}
                                                  </button>
                                                )}
                                              </div>
                                            );
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                      <p className="text-gray-400 text-sm italic">Aucune s├®ance n'est encore enregistr├®e pour ce client.</p>
                      {!client.module_id && <p className="text-xs text-rose-500 mt-2 font-bold">ÔÜá´©Å Assignez un module ├á ce client pour g├®n├®rer ses s├®ances.</p>}
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="col-span-full py-12 text-center bg-white rounded-3xl border-2 border-dashed border-gray-200">
            <p className="text-gray-400 font-medium italic">Aucun client ne vous est assign├® actuellement.</p>
          </div>
        )}
      </div>
    </div>
...
const DocumentsView = ({
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      ...
      <p className="text-gray-500 text-lg">Consultez et {isClient ? 'signez vos' : 'v├®rifiez les'} fichiers l├®gaux ou de synth├¿se.</p>
      ...
      {isAdmin && modelesTab === 'modeles' && (
        <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 mb-8">
          <div className="flex justify-between items-start">
            <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center">
              <span className="w-2 h-6 bg-amber-500 rounded-full mr-3"></span> Ma Mod├®loth├¿que
            </h2>
          ...
          <div className="mb-8 p-5 bg-amber-50 rounded-2xl border border-amber-100 flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1">
              <label className="block text-xs font-bold text-amber-800 uppercase mb-2">G├®n├®rer un nouveau type de document</label>
              <input
                type="text"
            ...
            {Object.keys(documentTemplates || {}).map(key => (
              <div key={key} className="p-4 border border-gray-100 bg-gray-50/50 rounded-2xl group hover:border-amber-500 transition-all">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] font-bold text-amber-600 uppercase tracking-tighter">Mod├¿le Actif</span>
                ...
                <p className="text-[10px] text-gray-500 truncate">{documentTemplates[key]?.name || "Mod├¿le charg├®"}</p>
              </div>
            ))}
            {Object.keys(documentTemplates || {}).length === 0 && (
              <div className="col-span-full py-4 text-center text-gray-400 italic text-sm">Aucun mod├¿le .docx enregistr├®.</div>
            )}
          </div>
        </div>
      )}
          ...
          <form onSubmit={handleAddDocument} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex-1">
                ...
                <select required value={newDocType} onChange={(e) => setNewDocType(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 w-full p-3 outline-none transition-all">
                  ...
                  <option value="├ëmargement">├ëmargement</option>
                  <option value="├ëvaluation">├ëvaluation</option>
                  <option value="Pr├®sence">Pr├®sence (S├®ance)</option>
                </select>
              </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-end">
              <div className="w-full lg:flex-1">
                <label className="block text-sm font-medium text-gray-700 mb-1">Client Rattach├®</label>
              ...
              <button type="submit" disabled={isAddingDoc} className="w-full lg:w-max bg-gray-900 hover:bg-gray-800 text-white font-medium rounded-xl px-6 py-3 shrink-0 disabled:opacity-50 transition-colors shadow-sm">
                {isAddingDoc ? "Ajout..." : "Cr├®er le document"}
              </button>
            </div>
          </form>
        </div>
        ...
        {isFormateur ? (
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-100 overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody>
                          ...
                          <tr key={doc.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors group">
                            <td className="py-4 px-2">
                              <div className="flex items-center">
                                ...
                                <div className="flex flex-col">
                                  ...
                                  {doc.type_document === 'Pr├®sence' && doc.date_seance && (
                                    <span className="text-[10px] text-gray-400 font-medium">S├®ance du : {new Date(doc.date_seance).toLocaleDateString('fr-FR')}</span>
                                  )}
                                  {doc.url && <button onClick={() => setViewingDocId(doc.id)} className="text-[10px] text-blue-600 font-bold hover:underline mt-0.5 text-left">Voir PDF Ôåù</button>}
                                </div>
                              </div>
                            </td>
                            <td className="py-4 px-2">
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${doc.signe_par_client ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}>B├®n├®ficiaire</span>
                            ...
                            <td className="py-4 px-2 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button onClick={() => handleDownloadPDF(doc)} className="p-1.5 border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-100 transition-colors" title="T├®l├®charger">
                                  <DownloadIcon />
                                </button>
                              </div>
                            </td>
                        ...
                        {clientDocs.length === 0 && (
                          <tr><td colSpan="3" className="py-8 text-center text-gray-400 italic text-sm">Aucun document pour ce b├®n├®ficiaire.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 mb-8 max-w-5xl mx-auto">
            ...
            <div className="flex flex-wrap gap-4 border-b border-gray-200 mb-6">
              <button onClick={() => setClientDocTab('avant')} className={`px-4 py-3 font-bold text-sm transition-colors ${clientDocTab === 'avant' ? 'border-b-2 border-rose-500 text-rose-500' : 'text-gray-500 hover:text-gray-800 border-b-2 border-transparent'}`}>├Ç signer avant d├®but</button>
            ...
            <div className="space-y-4">
              {(() => {
                ...
                const renderDocRow = (doc) => (
                  <div key={doc.id} className="p-4 border border-gray-100 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-rose-200 bg-gray-50/30">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {doc.signe_par_client && <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded border border-green-100">Ô£ô Votr├® signature Valid├®e</span>}
                      <button onClick={() => handleDownloadPDF(doc)} className="p-2 bg-white border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors shadow-sm" title="T├®l├®charger">
                        <DownloadIcon />
                      </button>
                    </div>
                  </div>
                ...
                if (clientDocTab === 'avant') {
                  ...
                  const items = clientDocs.filter(d => d.type_document === '├ëvaluation' || String(d.nom).toLowerCase().includes('attestation') || String(d.nom).toLowerCase().includes('bilan'));
                  return items.length > 0 ? items.map(renderDocRow) : <p className="text-sm text-gray-500 italic py-4">Les documents de fin de parcours appara├«tront ici.</p>;
                } else if (clientDocTab === 'supports') {
                  const supportDocs = clientDocs.filter(d => !['Contrat', '├ëvaluation'].includes(d.type_document) && !String(d.nom).toLowerCase().includes('contrat') && !String(d.nom).toLowerCase().includes('attestation') && !String(d.nom).toLowerCase().includes('convention'));
                  ...
                  return (
                    <div className="space-y-6">
                      {unlockedSessions.length > 0 && (
                        <div>
                          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Supports D├®bloqu├®s (S├®ances Pass├®es)</h3>
                          <div className="space-y-3">
                            {unlockedSessions.map(s => (
                              <div key={s.id} className="p-4 border border-rose-100 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 bg-rose-50/50">
                                <div className="flex items-center gap-3">
                                  <div className="w-10 h-10 bg-rose-100 text-rose-600 flex items-center justify-center rounded-xl shrink-0">­ƒôÜ</div>
                                  <div>
                      ...
                      {lockedSessions.length > 0 && (
                        <div>
                          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Acc├¿s Verrouill├®s (Sessions Futures)</h3>
                          <div className="space-y-3">
                            {lockedSessions.map(s => (
                              <div key={s.id} className="p-4 border border-gray-100 rounded-2xl flex items-center gap-4 bg-gray-50 opacity-60">
                                <div className="w-10 h-10 bg-gray-200 text-gray-500 flex items-center justify-center rounded-xl shrink-0">­ƒöÆ</div>
                                <div>
                                  <p className="font-bold text-gray-700 text-sm">{s.ressource_titre || `Support de la session ${s.titre}`}</p>
                                  <p className="text-[10px] text-gray-500">Sera d├®bloqu├® le {new Date(s.date).toLocaleDateString()}</p>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      {unlockedSessions.length === 0 && lockedSessions.length === 0 && supportDocs.length === 0 && (
                        <p className="text-sm text-gray-500 italic py-4">Aucun support p├®dagogique disponible pour le moment.</p>
                      )}
                    </div>
                  );
                }
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-200 text-sm text-gray-500">
                    ...
                    {!isClient && <th className="pb-3 font-medium">B├®n├®ficiaire</th>}
                    <th className="pb-3 font-medium">Visibilit├®</th>
                    ...
                    return (
                      <tr key={doc.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="py-4 font-bold text-gray-900 flex flex-col">
                          ...
                          {doc.url && <button onClick={() => setViewingDocId(doc.id)} className="inline-flex mt-2 items-center text-xs px-3 py-1.5 bg-blue-50 text-blue-700 font-bold rounded-lg hover:bg-blue-100 transition-colors w-fit shadow-sm border border-blue-200">Voir le document Ôåù</button>}
                        </td>
                        {!isClient && (
                          <td className="py-4 text-sm text-gray-600 font-medium">
                            {clientAssocie?.nom || 'Non d├®fini'}
                          </td>
                        )}
                        ...
                        <td className="py-4">
                          <div className="flex flex-col gap-1">
                            <span className={`px-2 py-0.5 rounded text-[11px] font-bold w-max border ${doc.signe_par_client ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-500 border-gray-200'
                              ...
                              Client: {doc.signe_par_client ? 'Ô£ô Sign├®' : '├Ç signer'}
                            </span>
                            <span className={`px-2 py-0.5 rounded text-[11px] font-bold w-max border ${doc.signe_par_formateur ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-50 text-gray-500 border-gray-200'
                              ...
                              Formateur: {doc.signe_par_formateur ? 'Ô£ô Sign├®' : '├Ç signer'}
                            </span>
                          </div>
                          {doc.type_document === 'Pr├®sence' && (
                            <div className="mt-2 text-xs text-gray-500 flex items-center gap-1">
                              <span title="Date de s├®ance">­ƒôà</span>
                              {isFormateur ? (
                                ...
                                <span className="font-bold text-gray-700 bg-gray-50 px-2 py-0.5 rounded border border-gray-200">{doc.date_seance ? new Date(doc.date_seance).toLocaleDateString() : 'Non d├®finie'}</span>
                              )}
                            </div>
                          )}
                        </td>
                        <td className="py-4 text-right space-x-2 whitespace-nowrap">
                          ...
                          {((isClient && doc.signe_par_client) || (isFormateur && doc.signe_par_formateur) || isAdmin) && (
                            <button onClick={() => handleDownloadPDF(doc)} className="inline-flex items-center text-sm px-3 py-1.5 border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors">
                              <span className="hidden lg:inline text-xs">T├®l├®charger</span>
...
const AccueilView = ({ setActiveTab, clientProgress }) => (
  <div className="flex flex-col items-center justify-center pt-10 md:pt-20 animate-fade-in">
    ...
    <p className="text-lg text-gray-500 max-w-lg text-center leading-relaxed">Bienvenue sur votre espace VB Coaching. Suivez votre progression Qualiopi et acc├®dez ├á vos s├®ances.</p>
    ...
    <div className="w-full max-w-3xl mt-8 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
      ...
      <p className="text-[10px] text-gray-400 mt-2 italic text-left">Mise ├á jour automatique par signature ├®lectronique (Qualiopi).</p>
    </div>
    ...
    <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 cursor-pointer hover:shadow-md transition-shadow" onClick={() => setActiveTab('mes_seances')}>
        <h3 className="font-bold text-gray-800 text-lg mb-2">Planning des S├®ances</h3>
        <p className="text-sm text-gray-500 mb-4">Consultez vos dates et ├®marger vos rapports de pr├®sence.</p>
        <div className="bg-rose-50 text-rose-700 px-4 py-2 rounded-xl text-sm font-bold text-center">Voir mes s├®ances</div>
      </div>
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 cursor-pointer hover:shadow-md transition-shadow" onClick={() => setActiveTab('bilan')}>
        <h3 className="font-bold text-gray-800 text-lg mb-2">Synth├¿se de Bilan</h3>
        <p className="text-sm text-gray-500 mb-4">Vos r├®sultats d'├®valuation ont ├®t├® trait├®s par notre approche IA.</p>
...
const SessionsView = ({
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Mes S├®ances d'Accompagnement</h1>
      <p className="text-gray-500 text-lg">Retrouvez le calendrier de vos 8 s├®ances et signez vos ├®margements.</p>

      <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-800 flex items-center">
            <span className="w-2 h-6 bg-gray-900 rounded-full mr-3"></span> Liste des S├®ances
          </h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {(() => {
                ...
                return sortedGroups.map((group, gIdx) => {
                  ...
                  const previousGroupNotSigned = gIdx > 0 && sortedGroups[gIdx - 1].items.some(s => s.statut !== 'Sign├®');
                    ...
                    <React.Fragment key={gIdx}>
                        ...
                        <td colSpan="4" className={`py-3 px-4 border-l-4 ${isLocked ? 'border-gray-300' : 'border-gray-900'}`}>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-[10px] font-bold text-gray-500">
                              {isLocked && <span className="bg-gray-200 text-gray-500 px-2 py-0.5 rounded mr-2">­ƒöÆ VERROUILL├ë</span>}
                              {group.date ? new Date(group.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' }) : 'Date ├á d├®finir'} ÔÇó {group.debut || '--:--'} - {group.fin || '--:--'}
                            </div>
                          </div>
                        </td>
                      </tr>
                      {group.items.map(session => (
                        ...
                        <td className="py-4 pl-12">
                          <div className="flex items-center gap-3">
                            <span className="text-lg">{session.type_activite === 'signature' ? 'Ô£ì´©Å' : session.type_activite === 'document' ? '­ƒôä' : 'ÔÜÖ´©Å'}</span>
                            <div className="flex flex-col">
                        ...
                        <td className="py-4 text-xs italic text-gray-400">
                          {session.ressource_id ? `Ressource : ${session.ressource_id}` : 'Pas de ressource li├®e'}
                        </td>
                        <td className="py-4 text-center">
                          <div className="flex flex-col gap-1 items-center">
                            <div className="flex items-center gap-1.5">
                              <span className={`w-1.5 h-1.5 rounded-full ${session.statut_client === 'Sign├®' ? 'bg-green-500' : 'bg-orange-400'}`}></span>
                              <span className="text-[9px] font-black uppercase text-gray-500">Moi: {session.statut_client || (session.statut === 'Sign├®' ? 'Sign├®' : '├Ç venir')}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className={`w-1.5 h-1.5 rounded-full ${session.statut_formateur === 'Sign├®' ? 'bg-green-500' : 'bg-orange-400'}`}></span>
                              <span className="text-[9px] font-black uppercase text-gray-500">Coach: {session.statut_formateur || (session.type_activite === 'signature' && session.statut === 'Sign├®' ? 'Sign├®' : '├Ç venir')}</span>
                            </div>
                          </div>
                        </td>
                          ...
                          <div className="flex justify-end gap-2">
                            {(() => {
                              ...
                              if (session.type_activite === 'signature') {
                                return (
                                  <button
                                    disabled={session.statut_client === 'Sign├®' || isDateLocked}
                                    onClick={() => signSession(session)}
                                    className={`text-[10px] font-bold px-3 py-1.5 rounded-lg border transition-all ${session.statut_client === 'Sign├®' ? 'bg-green-50 text-green-600 border-green-200' : isDateLocked ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-rose-500 text-white border-rose-600 hover:bg-rose-700'}`}
                                  >
                                    {session.statut_client === 'Sign├®' ? '├ëmarg├® Ô£ô' : isDateLocked ? `Indisponible` : '├ëmarger'}
                                  </button>
                                );
                              }
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    {isToSign && (
                                      <button
                                        disabled={session.statut_client === 'Sign├®' || isDateLocked}
                                        onClick={() => signSession(session)}
                                        className={`text-[10px] font-bold px-3 py-1.5 rounded-lg border transition-all ${session.statut_client === 'Sign├®' ? 'bg-green-50 text-green-600 border-green-200' : isDateLocked ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed' : 'bg-indigo-600 text-white border-indigo-700 hover:bg-indigo-800'}`}
                                      >
                                        {session.statut_client === 'Sign├®' ? 'Sign├® Ô£ô' : 'Signer Document'}
                                      </button>
                                    )}
                                  </div>
                                );
                              ...
                              if (session.type_activite === 'exercice' || session.type_activite === 'Exercice') {
                                return (
                                  <div className="flex gap-2">
                                    ...
                                    <label className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-emerald-600 text-white cursor-pointer hover:bg-emerald-700 transition-colors">
                                      {session.statut === 'Rendu' ? 'Modifier R├®ponse' : 'Soumettre R├®ponse'}
                                      <input
                                        type="file"
                                        className="hidden"
                                        onChange={(e) => e.target.files[0] && handleUploadExerciseResponse(session.id, e.target.files[0])}
              ...
              {mySessions.length === 0 && (
                <tr>
                  <td colSpan="4" className="py-12 text-center text-gray-400 italic">Aucune s├®ance n'est encore programm├®e. Votre coach les g├®n├®rera prochainement.</td>
                </tr>
              )}
            </tbody>
          </table>
  ...
  <div className="space-y-6 max-w-5xl mx-auto">
    <div className="flex flex-col md:flex-row md:items-center justify-between">
      <div>
        ...
        <p className="text-gray-500 text-lg mt-1">Retrouvez la synth├¿se de vos ├®valuations.</p>
      </div>
      <button onClick={handleDownloadPDF} className="mt-4 md:mt-0 bg-gray-900 hover:bg-gray-800 text-white px-5 py-2.5 rounded-xl font-medium shadow-md transition-all text-sm flex items-center w-fit">
        <DownloadIcon /> T├®l├®charger la synth├¿se
      </button>
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
        <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center"><span className="w-2 h-6 bg-rose-500 rounded-full mr-3"></span>Mes Int├®r├¬ts Dominants</h2>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5 hover:bg-emerald-100 transition-colors group">
            ...
            <p className="text-xs text-emerald-700 mt-1">Go├╗t prononc├® pour la technique.</p>
          </div>
        </div>
      </div>
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 hover:shadow-md flex flex-col items-center">
        <h2 className="text-xl w-full font-bold text-gray-800 flex items-center mb-2"><span className="w-2 h-6 bg-rose-500 rounded-full mr-3"></span>Mes Ancres de Carri├¿re</h2>
        ...
        <p className="text-sm text-gray-600 bg-gray-50 py-2 px-4 rounded-lg w-full text-center">Dominantes : <strong className="text-gray-900">S├®curit├® (5.8)</strong> et <strong className="text-gray-900">Technique (5.5)</strong></p>
      </div>
    </div>
  </div>
);
...
const ExercicesView = ({ setActiveTab }) => (
  <div className="space-y-6 animate-fade-in relative h-[calc(100vh-140px)] flex flex-col max-w-5xl mx-auto">
    <div className="flex justify-between items-center mb-6">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Outils P├®dagogiques</h1>
        <p className="text-gray-500 text-lg">Retrouvez les ├®valuations actives et pass├®es.</p>
      </div>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full cursor-pointer" onClick={() => setActiveTab('bilan')}>
      <div className="bg-white border-2 border-green-100 rounded-3xl p-6 relative overflow-hidden group hover:shadow-md transition-shadow">
        ...
        <h3 className="text-xl font-bold text-gray-800 mb-2">Test des Ancres de Carri├¿re</h3>
        <p className="text-gray-500 text-sm mb-4">Exercice de 40 questions. Permet de d├®finir vos dominantes professionnelles.</p>
        <span className="bg-green-100 text-green-800 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">Ô£ö Termin├®</span>
      </div>
    </div>
  </div>
);
...
const ProfileView = ({ currentUserId, supabase, fetchUtilisateurs, formateurs, clients, userRole }) => {
  ...
  const handleUpdate = async () => {
    ...
    if (!error) {
      alert('Profil mis ├á jour avec succ├¿s !');
      fetchUtilisateurs();
    } else {
      alert('Erreur lors de la mise ├á jour : ' + error.message);
    }
  };
        ...
        <div className="space-y-5">
          ...
          {(userRole === 'admin' || userRole === 'formateur') && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-2">NDA (Num├®ro D├®claration Activit├®)</label>
                <input
                  className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
...
const RessourcesView = ({ pedagogicalResources, supabase }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Ressources P├®dagogiques</h1>
          <p className="text-gray-500 text-lg">Biblioth├¿que de documents partag├®s par l'administration.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pedagogicalResources.map(res => (
            <div key={res.name} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
              ...
              <p className="text-[10px] text-gray-400">Ajout├® le {new Date(res.created_at).toLocaleDateString()}</p>
...
const SetPasswordView = ({ supabase, onComplete }) => {
  ...
  // Au montage : v├®rifier le token dans l'URL via verifyOtp
  useEffect(() => {
    const verifyToken = async () => {
      ...
      if (tokenHash && type) {
        // ├ëchanger le token_hash contre une session Supabase
        ...
        if (error) {
          ...
          setVerifyError('Le lien est invalide ou a expir├®. Demandez un nouveau lien ├á votre administrateur.');
        }
      }
      // Si pas de token_hash, on cherche access_token (ancien flux)
      // Supabase le g├¿re automatiquement via onAuthStateChange
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 w-full max-w-md animate-fade-in text-center">
          ...
          <p className="text-gray-500 font-medium">V├®rification de votre lien en cours...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 w-full max-w-md animate-fade-in text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Lien expir├®</h2>
          <p className="text-gray-500 text-sm mb-6">{verifyError}</p>
          <button onClick={() => { window.history.replaceState(null, '', '/'); window.location.reload(); }} className="bg-gray-900 text-white font-bold py-3 px-6 rounded-xl">Retour ├á l'accueil</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 w-full max-w-md animate-fade-in">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2 text-center">Finalisez votre acc├¿s</h2>
        <p className="text-gray-500 text-sm mb-6 text-center">Cr├®ez votre mot de passe pour acc├®der ├á VBERP.</p>
        <form onSubmit={handleSetPassword} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="password"
              required
              placeholder="Saisissez ├á nouveau votre mot de passe"
          ...
          >
            {isUpdating ? 'Configuration en cours...' : 'Valider et acc├®der ├á mon espace'}
          </button>
        </form>
      </div>
    </div>
...
const ResetPasswordPage = ({ supabase, onComplete }) => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      if (tokenHash && type === 'recovery') {
        ...
        if (error) {
          setVerifyError('Lien expir├® ou invalide. Veuillez refaire une demande de r├®initialisation.');
        }
      }
      setIsVerifying(false);
    };
  ...
  const handleUpdatePassword = async (e) => {
    ...
    if (!error) {
      ...
      setErrorMsg('Erreur lors de la mise ├á jour : ' + error.message);
    }
    setIsUpdating(false);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 w-full max-w-md animate-fade-in">
        ...
        <p className="text-gray-500 text-sm mb-6 text-center">Cr├®ez votre nouveau mot de passe s├®curis├®.</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl p-8 shadow-2xl border border-gray-100 w-full max-w-md animate-scale-up relative">
        <button onClick={onClose} className="absolute top-6 right-6 text-gray-400 hover:text-gray-600">Ô£ò</button>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Inviter un Utilisateur</h2>
        <p className="text-gray-500 text-sm mb-8">Un email sera envoy├® pour d├®finir le mot de passe.</p>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onInvite(formData); }} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase mb-2">R├┤le</label>
            <select
              className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500"
              value={formData.role}
              onChange={e => setFormData({ ...formData, role: e.target.value })}
            >
              <option value="client">Client (B├®n├®ficiaire)</option>
          ...
          {formData.role === 'client' && (
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Formateur R├®f├®rent</label>
              <select
                required
                className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500"
...
export default function App() {
  // --- ├ëtats Session et Navigation ---
  const [userRole, setUserRole] = useState(null); // 'admin' | 'formateur' | 'client' | null
  const [currentUserId, setCurrentUserId] = useState(null);
  const [activeTab, setActiveTab] = useState('accueil');
  const [isLoadingSession, setIsLoadingSession] = useState(true);
  const [isMobileMenuOpen, setMobileMenuOpen] = useState(false);
  // D├®tection synchrone au d├®marrage : lien invitation (token_hash ou access_token dans le hash)
  ...
  useEffect(() => {
    const initSession = async () => {
      ...
      if (session?.user?.email) {
        console.log('[App] Session trouv├®e pour:', session.user.email);
  ...
  // ├ëtats formulaire "Ajouter un compte"
  ...
  // ├ëtats formulaire "Ing├®nierie Modules" (Admin)
  ...
  // ├ëtats formulaire "├ëtape de Parcours"
  ...
  // ├ëtats formulaire "Ajouter un document"
  ...
  const fetchDocuments = async () => {
    // 1. Charger les documents classiques (contrats g├®n├®r├®s, preuves, etc.)
    const { data: docsData, error } = await supabase.from('documents').select('*');
    if (!error && docsData) setDocuments(docsData);

    // 2. Charger les mod├¿les ma├«tres depuis la table unifi├®e module_step_resources
    const { data: modsData, error: modErr } = await supabase.from('module_step_resources').select('*');
    if (!modErr && modsData) {
      console.log(`[fetchDocuments] ${modsData.length} ressources r├®cup├®r├®es avec succ├¿s dans module_step_resources.`);
      const templates = {};
      modsData.forEach(m => {
      ...
      console.error("[fetchDocuments] Erreur lors de la r├®cup├®ration des ressources mod├¿les :", modErr);
      // Fallback
      const refs = docsData?.filter(d => d.type_document === 'Mod├¿le R├®f├®rence') || [];
      const templates = {};
      refs.forEach(r => {
  ...
  const handleUploadResource = async (file) => {
    ...
    if (!error) {
      alert('Ressource p├®dagogique ajout├®e avec succ├¿s !');
      setNewResourceName('');
  ...
  const handleInviteUser = async (formData) => {
    ...
    // 1. Cr├®er un client admin avec la service_role_key
    const serviceKey = process.env.REACT_APP_SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceKey) {
      alert("Cl├® de service non configur├®e. V├®rifiez REACT_APP_SUPABASE_SERVICE_ROLE_KEY dans Vercel.");
    ...
    if (inviteError) {
      ...
      alert(`L'invitation a ├®chou├® : ${inviteError.message}`);
      setIsAddingUser(false);
      return;
    }
    ...
    // 3. Cr├®er l'entr├®e dans la table correspondante
    if (role === 'client') {
      // Pour les clients : on utilise la table 'clients' avec l'UUID de l'Auth
      // CRUCIAL : on utilise adminClient pour outrepasser les r├¿gles RLS lors de l'insertion initiale
      ...
      if (dbError) {
        console.error("Erreur DB clients apr├¿s invitation:", dbError);
      ...
      if (dbError) console.error("Erreur DB utilisateurs apr├¿s invitation:", dbError);
    }

    alert(`Ô£à Invitation envoy├®e par email ├á ${email} !`);
    setIsAddingUser(false);
  ...
  // --- Suppression S├®curis├®e (Cascade & Auth) ---
  const handleDeleteClient = async (clientId) => {
    try {
      // 1. Supprimer les s├®ances
      ...
      alert("Client et toutes ses donn├®es supprim├®s avec succ├¿s.");
      ...
      // On continue m├¬me si deleteUser ├®choue (ex: pas de compte auth)
      setExpandedClientId(null);
      fetchUtilisateurs();
    }
  };
  ...
  const handleDeleteFormateur = async (formateurId) => {
    try {
      ...
      alert("Formateur supprim├® avec succ├¿s.");
      fetchUtilisateurs();
    } catch (err) {
  ...
  const handleAddUser = async (e) => {
    ...
    if (newUserRole === 'client') {
      const { error: clientError } = await supabase
        ...
        .insert([{
          ...
          formateur_id: null // Pas de formateur s├®lectionn├® pour l'ajout manuel simple
        }]);
      error = clientError;
    } else {
    ...
    if (error) {
      ...
      alert(`Utilisateur ${newUserName} ajout├® avec succ├¿s !`);
      await fetchUtilisateurs();
  ...
  const handleModuleChange = async (clientId, moduleId) => {
    console.log('[handleModuleChange] D├®but. clientId:', clientId, 'moduleId:', moduleId);
    ...
    console.log('[handleModuleChange] Update r├®ussi. R├®cup├®ration du client pour v├®rification...');
    const { data: updatedClient, error: fetchErr } = await supabase.from('clients').select('*').eq('id', clientId).single();
    if (fetchErr) {
      console.error("[handleModuleChange] Erreur lors de la r├®cup├®ration du client mis ├á jour:", fetchErr);
      return;
    }

    console.log('[handleModuleChange] Client mis ├á jour r├®cup├®r├®:', JSON.stringify(updatedClient, null, 2));

    if (updatedClient && finalModuleId) {
      // Mapping pour compatibilit├® avec generateSessions (nom_complet -> nom)
      const compatibleClient = {
        ...updatedClient,
        nom: updatedClient.nom_complet || updatedClient.nom || updatedClient.email || 'B├®n├®ficiaire',
        id: updatedClient.id,
        module_id: finalModuleId
      };

      console.log('[handleModuleChange] D├®clenchement de generateSessions...');
      // 3. D├®clenchement automatique des sessions (Qualiopi)
  ...
  const handleLinkDocument = async (e, selectedModule) => {
    ...
    if (error) {
      console.error("D├®tail de l'erreur RPC Supabase :", error);
  ...
  const createSessionFolder = async (moduleId, title) => {
    ...
    if (error) {
      console.error("Erreur cr├®ation dossier Supabase:", error);
      alert("Erreur lors de la cr├®ation du dossier : " + error.message);
    } else {
      console.log("Dossier cr├®├® avec succ├¿s, lancement du rafra├«chissement (fetchModules)");
      setNewStepTitle('');
      await fetchModules();
    }
  ...
  const handleAddStepResource = async (templateId, stepData) => {
    ...
    const { error } = await supabase.from('module_step_resources').insert([{
      ...
      titre: stepData.title || (stepData.type === 'signature' ? '├ëmargement' : 'Document'),
      type: stepData.type,
      // On s├®pare l'UUID (ressource_id) du chemin texte (file_url)
  ...
  const generateSessions = async (client) => {
    // Client ID est syst├®matiquement un UUID (String)
    const finalClientId = client.id;
    console.log("Tentative de g├®n├®ration pour le client:", finalClientId);

    // Module ID est syst├®matiquement un BigInt (Number)
    const finalModuleId = Number(client.module_id);

    console.log(`[generateSessions] D├®but. ClientID: ${finalClientId}, ModuleID: ${finalModuleId}`);
    ...
    // R├®cup├®rer l'objet module correspondant
    const module = modules.find(m => String(m.id) === String(moduleId));
    if (!module) {
      console.error("[generateSessions] Module non trouv├® dans l'├®tat local pour l'id:", moduleId, "Modules dispos:", modules.map(m => m.id));
      return;
    }

    // Verrou de concurrence local pour emp├¬cher que l'effet ne s'ex├®cute plusieurs fois simultan├®ment
    ...
    try {
      ...
      console.log('[generateSessions] Templates r├®cup├®r├®s:', templates?.length || 0, 'Erreur:', tempError);
      ...
      // 2. V├®rifier les s├®ances existantes pour ce client (UUID ou BigInt)
      ...
      if (checkError) {
        console.error('Erreur v├®rification s├®ances:', checkError);
        return;
      }
      ...
      if (templates && templates.length > 0) {
        // Nouvelle logique Imbriqu├®e (Keyro)
        for (const t of templates) {
          ...
          console.log(`[generateSessions]   -> ${resources?.length || 0} ressources trouv├®es pour template ${t.id}`);
          ...
          if (resources && resources.length > 0) {
            resources.forEach(res => {
              sessionsToInsert.push({
                ...
                statut: '├Ç venir',
                statut_client: '├Ç venir',
                statut_formateur: '├Ç venir'
              });
            });
          } else {
            // Dossier vide
        ...
        // Fallback backward compatibility : g├®n├®ration par nombre de s├®ances pr├®vues
        for (let i = 1; i <= module.seances_prevues; i++) {
          const defaultTitle = `${module.nom} - S├®ance ${i}`;
      ...
      // 2. D├®duplication manuelle pour ├®viter l'erreur 42P10 (absence d'index unique ON CONFLICT)
      console.log('[generateSessions] R├®cup├®ration des s├®ances existantes pour d├®duplication...');
      ...
      if (fetchSessionsErr) {
        console.error('[generateSessions] Erreur lors de la r├®cup├®ration pour d├®duplication:', fetchSessionsErr);
        // On continue quand m├¬me, au risque de cr├®er des doublons si le fetch ├®choue
      }
      ...
      console.log(`[generateSessions] Apr├¿s d├®duplication : ${finalSessionsToInsert.length} s├®ances ├á ins├®rer.`);

      if (finalSessionsToInsert.length === 0) {
        console.log('[generateSessions] Toutes les s├®ances existent d├®j├á. Pas d\'insertion n├®cessaire.');
      ...
      if (!error) {
        console.log(`[generateSessions] SUCC├êS: ${finalSessionsToInsert.length} s├®ance(s) ins├®r├®e(s).`);
        if (typeof fetchSessions === 'function') await fetchSessions();
      } else {
        console.error('[generateSessions] ERREUR insertion finale :', error);
        console.error('[generateSessions] Payload tent├®:', finalSessionsToInsert);
      }
    } finally {
  ...
  const handleAddSession = async (client) => {
    ...
    const moduleName = module ? module.nom : "S├®ance";

    const { error: insError } = await supabase.from('sessions').insert([{
      numero_seance: nextNum,
      nom: `${moduleName} - S├®ance ${nextNum}`,
      client_id: client.id,
      module_id: client.module_id,
      metadata: {},
      statut: '├Ç venir',
    ...
    if (!insError) {
      ...
      alert("Erreur lors de l'ajout de la s├®ance: " + insError.message);
    }
  };
  ...
  const handleDeleteSession = async (session) => {
    ...
    if (!window.confirm(`Supprimer la s├®ance N┬░${session.numero_seance} de ${client.nom} ?`)) return;
  ...
  const handleSessionSignatureSave = async (sessionId, signatureDataUrl) => {
    ...
    if (userRole === 'formateur' || userRole === 'admin') {
      ...
      updateData.statut_formateur = 'Sign├®';
      // La session devient verte si les deux ont sign├®
      if (session.statut_client === 'Sign├®') {
        updateData.statut = 'Sign├®';
      }
    } else {
      updateData.signature_image = signatureDataUrl;
      updateData.date_signature = new Date().toISOString();
      updateData.statut_client = 'Sign├®';
      // La session devient verte si les deux ont sign├®
      if (session.statut_formateur === 'Sign├®') {
        updateData.statut = 'Sign├®';
      }
    }
    ...
    if (!error) {
      ...
      await fetchSessions();
      alert(`S├®ance ├®marg├®e avec succ├¿s !`);
    } else {
  ...
  const handleUploadDocxTemplate = async (fileArg, typeArg) => {
    try {
      ...
      if (!file) {
        alert("Veuillez s├®lectionner un fichier .docx d'abord.");
        return;
      }
      if (!type) {
        alert("Veuillez saisir un nom pour ce type de mod├¿le.");
        return;
      }
      ...
      // Sauvegarder dans module_step_resources (Source unifi├®e)
      ...
      // Sync avec documents
      const { data: existingDoc } = await supabase.from('documents').select('id').eq('nom', type).eq('type_document', 'Mod├¿le R├®f├®rence');
      if (existingDoc && existingDoc.length > 0) {
        await supabase.from('documents').update({ url: publicUrl }).eq('id', existingDoc[0].id);
      } else {
        await supabase.from('documents').insert([{
          nom: type,
          type_document: 'Mod├¿le R├®f├®rence',
          url: publicUrl,
      ...
      await fetchDocuments();
      alert(`Mod├¿le Word pour "${type}" enregistr├® avec succ├¿s.`);
    } catch (err) {
  ...
  const handleDownloadResource = async (fileName) => {
    try {
      // Si le fileName est d├®j├á une URL compl├¿te (mod├®loth├¿que), on l'ouvre
      ...
      if (error || !data) throw error || new Error("Lien non g├®n├®r├®");
      window.open(data.signedUrl, '_blank');
    } catch (err) {
      console.error("Erreur t├®l├®chargement:", err);
      alert('Erreur lors du t├®l├®chargement : ' + (err.message || 'Fichier introuvable'));
    }
  };
  ...
  const handleGenerateDocx = async (clientRow, type) => {
    try {
      ...
      if (!templateInfo || !templateInfo.url) {
        alert(`Aucun mod├¿le .docx trouv├® pour "${type}". Veuillez l'uploader dans l'onglet Ing├®nierie.`);
        return;
      }

      // 1. R├®cup├®ration depuis la table cibl├®e 'clients'
      ...
      // 2. R├®cup├®ration formateur dans 'utilisateurs'
      let theCoach = { nom: 'Non assign├®' };
      ...
      if (!finalClient.module_id && !clientRow.module_id) {
        alert("Veuillez d'abord assigner un module ├á ce client.");
        return;
      }

      const module = modules.find(m => m.id === (finalClient.module_id || clientRow.module_id));

      // Extraction imp├®rative des dates via requ├¬te sur sessions
      ...
      let dateDebut = '[Date non d├®finie]';
      let dateFin = '[Date non d├®finie]';
      ...
      const fallbackNomFormateur = 'Nom du formateur non renseign├®';
      doc.setData({
      ...
      if (!uploadError) {
        ...
        await fetchDocuments();
        alert(`Document "${finalFileName}" g├®n├®r├® et archiv├®.`);
      }
    } catch (error) {
  ...
  const handleAddDocument = async (e) => {
    ...
    if (!error) {
      // Rechargement imm├®diat pour r├®cup├®rer le document dans l'UI
      await fetchDocuments();
      setNewDocName('');
      setNewDocType('Autre');
      setNewDocUrl('');
  ...
  const handleSignDocument = async (docId, signerType, signatureDataUrl = null) => {
    ...
    if (signatureDataUrl) {
      // Sauvegarde de l'image en Base64 directement dans les colonnes comme demand├®
    ...
    // Met ├á jour Supabase, mais aussi on emp├¬che les clics multiples
  ...
  const handleDownloadPDF = async (doc) => {
    if (doc && doc.id) {
      ...
      if (!urlToDownload) return alert("Aucun fichier ├á t├®l├®charger");
      ...
      try {
        ...
        if (!res.ok) throw new Error(`Impossible de r├®cup├®rer le document (${res.status})`);
        ...
        const clientName = client ? client.nom : 'B├®n├®ficiaire';
        ...
        const renderSigBlock = (role, imageBase64, name) => {
          const imageHtml = imageBase64
            ...
            : '<div style="width:280px;height:120px;border:1px dashed #9ca3af;display:flex;align-items:center;justify-content:center;color:#6b7280;font-size:12px;">Aucune signature attach├®e</div>';
          ...
          return `
            <div style="flex:1; min-width:280px; padding: 14px; border: 1px solid #d1d5db; border-radius: 12px; margin: 8px;">
              ...
              <p style="font-size: 12px; margin-bottom: 10px;">${dateText} ├á ${timeText}</p>
              ${imageHtml}
            </div>
          `;
        };
        ...
        certEl.innerHTML = `
          <div style="padding: 20px; border: 1px solid #d1d5db; border-radius: 12px;">
            <h1 style="font-size: 24px; margin-bottom: 16px;">Certificat de signature ├®lectronique</h1>
            ...
            <p style="font-size: 11px; margin-top: 30px; color:#4b5563;">Mentions l├®gales : Ce document fait foi de preuve d'├®margement pour Qualiopi, certifi├® par les deux parties.</p>
          </div>
        `;

        document.body.appendChild(certEl);

        // Petite pause pour ├¬tre s├╗r que le rendu image (Base64) est bien inject├®
        ...
        console.error('Erreur de g├®n├®ration du PDF final :', error);
        alert('Impossible de g├®n├®rer le PDF sign├®. V├®rifiez la console.');
      }
    } else {
      alert('Simulation : T├®l├®chargement du bilan...');
    }
  };



  // --- Chargement des donn├®es au lancement (Supabase) ---
  useEffect(() => {
    fetchUtilisateurs();
    fetchDocuments();
    fetchModules();
    ...
    // D├®tection des liens d'invitation ou de r├®cup├®ration de mot de passe
  ...
  // Auto-g├®n├®ration supprim├®e : elle d├®clenchait generateSessions 3x lors du chargement des donn├®es.
  // Les s├®ances sont d├®sormais g├®n├®r├®es uniquement via le bouton manuel dans la vue Formateur.
    ...
    return (
      <SetPasswordView
        ...
        onComplete={async () => {
          ...
          console.log('Auth user apr├¿s SetPassword:', user, authError);

          if (user && user.email) {
            // Chercher le r├┤le dans la base de donn├®es
            const { data: userData, error: dbError } = await supabase
              .from('utilisateurs')
              .select('role, id')
              .eq('email', user.email)
            ...
            if (userData && userData.role) {
              // ÔÜí D'ABORD d├®finir le r├┤le, PUIS masquer le formulaire de mot de passe
              handleLogin(userData.role, userData.id);
          ...
          // Fallback: pas de r├┤le trouv├®
          console.warn('Impossible de d├®terminer le r├┤le automatiquement.');
          setIsSettingPassword(false);
        }}
      />
    );
  ...
  if (isResetPassword) {
    return (
      <ResetPasswordPage
        supabase={supabase}
        onComplete={async () => {
          // On ne fait pas de log out car l'utilisateur est nouvellement authentifi├®
          // mais on peut signer la session out pour forcer un vrain login, ou juste le renvoyer
          await supabase.auth.signOut();
          window.history.replaceState(null, '', '/');
          setIsResetPassword(false);
          setResetSuccessMsg("Votre mot de passe a ├®t├® r├®initialis├® avec succ├¿s. Vous pouvez maintenant vous connecter.");
        }}
      />
    );
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      {/* Navigation Sidebar (Dynamic par R├┤le) */}
        ...
        <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
          {userRole === 'admin' && (
            <>
              ...
              <button onClick={() => { setActiveTab('mod├®loth├¿que'); setMobileMenuOpen(false); }} className={`w-full flex items-center px-4 py-3.5 rounded-xl transition-all duration-200 ${activeTab === 'mod├®loth├¿que' ? 'bg-rose-500 text-white shadow-lg' : 'hover:bg-gray-800 hover:text-white font-medium'}`}>
                <FileText className="w-5 h-5 mr-3" /> Mod├®loth├¿que
              </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="hidden md:flex bg-white px-10 py-5 border-b border-gray-100 shadow-sm z-10 justify-between items-center w-full shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <span className="bg-green-50 text-green-700 text-xs font-bold px-2 py-1 rounded-md border border-green-200">Connect├® en ligne</span>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="text-right mr-2">
              <p className="text-sm font-bold text-gray-800 leading-tight">
                ...
                {userRole === 'client' && (clients.find(c => c.id === currentUserId)?.nom || "B├®n├®ficiaire")}
              </p>
            </div>
            <div
        ...
        <main className="flex-1 overflow-y-auto bg-gray-50/50 p-6 md:p-10 w-full h-full">
          ...
          {activeTab === 'mod├®loth├¿que' && <DocumentsView
            sessions={sessions}
            documents={documents}
            clients={clients}
            formateurs={formateurs}
...
const FormateurDetailView = ({ formateur, onBack, supabase, fetchUtilisateurs, modules, clients, handleDeleteFormateur }) => {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      await fetchUtilisateurs();
      alert("Informations l├®gales enregistr├®es !");
    }
    setIsSaving(false);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      <button onClick={onBack} className="text-gray-500 hover:text-gray-900 font-bold flex items-center mb-4 transition-colors">
        <ChevronLeft className="w-5 h-5 mr-1" /> Retour ├á la liste des formateurs
      </button>
      ...
      <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-6 mb-8 pb-8 border-b border-gray-100">
          ...
          <div>
            ...
            <p className="text-gray-500 text-lg">Profil Formateur & Informations L├®gales</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
              <FileText className="text-rose-500" /> Identit├® Professionnelle
            </h3>
            <div className="space-y-4">
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-rose-500 transition-all"
                    value={legalInfo.formateur_nda} onChange={e => setLegalInfo({ ...legalInfo, formateur_nda: e.target.value })} placeholder="N┬░ D├®claration" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
              <Plus className="text-indigo-500" /> Coordonn├®es & Contact
            </h3>
            <div className="space-y-4">
              <div>
                ...
                <textarea rows="2" className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-rose-500 transition-all"
                  value={legalInfo.adresse_formateur} onChange={e => setLegalInfo({ ...legalInfo, adresse_formateur: e.target.value })} placeholder="Adresse compl├¿te du si├¿ge" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Email</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-2">T├®l├®phone</label>
                  <input className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-rose-500 transition-all"
        ...
        <div className="mt-12 flex justify-end items-center gap-4">
          ...
          <button onClick={handleSave} disabled={isSaving} className="bg-rose-600 hover:bg-rose-700 text-white font-bold py-4 px-10 rounded-2xl shadow-xl transition-all flex items-center gap-3 disabled:opacity-50">
            <Save size={20} />
            {isSaving ? 'Enregistrement...' : 'Enregistrer les informations l├®gales'}
          </button>
        </div>
      ...
      <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm mt-8">
        <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
          <Users className="text-rose-500" /> Clients Assign├®s
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div key={client.id} className="p-4 border border-gray-50 rounded-2xl bg-gray-50/50 hover:bg-white hover:border-rose-200 transition-all group">
                ...
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <span className={`px-2 py-1 rounded-lg text-[10px] font-bold ${assignedModule ? 'bg-rose-50 text-rose-600' : 'bg-gray-100 text-gray-400'}`}>
                    {assignedModule ? assignedModule.nom : 'Pas de module assign├®'}
                  </span>
                </div>
              </div>
            );
          ...
          {clients.filter(c => c.formateur_id === formateur.id).length === 0 && (
            <p className="text-gray-400 italic text-sm py-4">Aucun client n'est encore assign├® ├á ce formateur.</p>
          )}
        </div>
      </div>
    </div>
