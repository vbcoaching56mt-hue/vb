// api/prospects/envoyer.js
//
// Envoie un questionnaire d'entretien préalable à un prospect qui n'a PAS de compte SkorUp —
// demandé par l'utilisateur le 22/09/2026. Le formateur (ou l'admin) saisit nom/prénom/email
// et choisit un modèle ; cette fonction crée l'envoi en base avec un token aléatoire
// impossible à deviner, puis envoie l'email contenant le lien public.
//
// Reprend exactement le même schéma de sécurité que api/formateur/notify-assignment.js :
//   - Fonction serverless (jamais exposée au navigateur) : RESEND_API_KEY et
...
//     navigateur), puis vérifié comme étant bien admin OU formateur d'un organisme (avec le
//     même correctif auth_uid / repli par email que trigger-manual.js et notify-assignment.js).
//   - Le modèle de questionnaire (templateId) est revérifié comme appartenant à CET organisme
//     et actif — jamais de confiance aveugle dans l'id envoyé par le navigateur.
//
// Le token du lien public est généré ici avec crypto.randomBytes(32) (256 bits d'aléatoire) :
// c'est ce token, pas une policy RLS, qui protège l'accès du prospect à SES réponses uniquement
// (voir prospect_questionnaires_migration.sql pour le détail du choix de sécurité).
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── 2. Vérifier que l'appelant est bien admin OU formateur d'un organisme ──
    ...
    // ── 3. Lire et valider les champs envoyés ──
    ...
    // ── 4. Revérifier le modèle comme appartenant à CET organisme et actif ──
    ...
    // ── 5. Créer l'envoi avec un token aléatoire (256 bits) ──
    ...
    // ── 6. Envoyer l'email via Resend (appel fetch direct, même approche que trigger-manual.js) ──
    ...
    console.error('[prospects/envoyer]', err);
